    ...
    <div class="panel-body">
     <table class="table table-striped table-bordered">
      ...
      <c:forEach var="tempEmployee" items="${employees}">
       ...
       <!-- construct an "update" link with employee id -->
       <c:url var="createLink" value="/request/showForm">
        <c:param name="employeeId" value="${tempEmployee.id}" />
       </c:url>

       <tr>
        <td>${tempEmployee.admissionDate}</td>
        <td>${tempEmployee.name}</td>
        <td>${tempEmployee.salary}</td>

        <td>
        <!-- display the update link --> <a href="${createLink}">Create</a>
